...
async function fetchFromClipchamp(endpoint: string, options: RequestInit = {}): Promise<Response> {
...
async function getEditorState(): Promise<ToolResult> {
  try {
...
async function sendCommand(command: string, params: Record<string, unknown>): Promise<ToolResult> {
  try {
    const response = await fetchFromClipchamp('/api/editor/state', {
      method: 'POST',
...
async function callMCPTool(toolName: string, args: Record<string, unknown>): Promise<ToolResult> {
  try {
    ...
    const data = await response.json() as { error?: { message: string }; result?: { content?: Array<{ type: string; text?: string }> } };
    if (data.error) {
...
async function addQwenTask(prompt: string, mode: 'image' | 'video', aspectRatio: string): Promise<ToolResult> {
  ...
  try {
    ...
    const data = await response.json() as { success: boolean; taskId?: string };
...
async function getQwenTaskStatus(taskId: string): Promise<ToolResult> {
  try {
    ...
    const data = await response.json() as { task?: QwenTask };
  ...
  switch (name) {
    ...
    case 'text_to_image':
      return addQwenTask(
        ...
        'image',
        (args.aspectRatio as string) || '9:16'
      );
    case 'text_to_video':
      return addQwenTask(
        args.prompt as string,
    ...
    case 'check_qwen_status':
      return getQwenTaskStatus(args.taskId as string);
    case 'add_generated_content': {
      const result = await addQwenTask(
        args.prompt as string,
        args.type as 'image' | 'video',
        (args.aspectRatio as string) || '9:16'
      );
